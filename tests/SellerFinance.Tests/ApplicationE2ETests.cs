...
public sealed class ApplicationE2ETests : IClassFixture<SellerFinanceApplicationFactory>
{
    ...
    public async Task Register_Settings_Expense_Tenant_Isolation_And_Delete_Flow_Works()
    {
        ...
        var feeRuleResponse=await client.PostAsJsonAsync("/api/v1/fee-rules",new{scope="Category",valueType="Percentage",value=10,effectiveFrom="2026-08-01",category=" Home "});Assert.Equal(HttpStatusCode.Created,feeRuleResponse.StatusCode);var feeRuleId=(await feeRuleResponse.Content.ReadFromJsonAsync<JsonElement>()).GetProperty("id").GetGuid();Assert.Equal(HttpStatusCode.OK,(await client.PutAsJsonAsync($"/api/v1/fee-rules/{feeRuleId}/end",new{effectiveTo="2026-08-31"})).StatusCode);Assert.Equal(HttpStatusCode.Conflict,(await client.PostAsJsonAsync("/api/v1/fee-rules",new{scope="Category",valueType="Percentage",value=12,effectiveFrom="2026-08-15",effectiveTo="2026-09-15",category="Home"})).StatusCode);Assert.Equal(HttpStatusCode.Created,(await client.PostAsJsonAsync("/api/v1/fee-rules",new{scope="Category",valueType="Percentage",value=12,effectiveFrom="2026-09-01",category="Home"})).StatusCode);await using(var scope=factory.Services.CreateAsyncScope()){var db=scope.ServiceProvider.GetRequiredService<SellerFinanceDbContext>();var rule=await db.FeeRules.SingleAsync(x=>x.Id==feeRuleId);Assert.Equal("Home",rule.Category);Assert.Equal(new DateOnly(2026,8,31),rule.EffectiveTo);Assert.True(await db.AuditLogs.AnyAsync(x=>x.OrganizationId==organizationId&&x.Action=="fee.rule.ended"&&x.EntityId==feeRuleId.ToString()));}
    ...
    public async Task Onboarding_Kaspi_Sync_Cost_Import_Dashboard_And_Export_Works_End_To_End()
    {
        ...
        var sync=await client.GetFromJsonAsync<JsonElement>($"/api/v1/kaspi/sync/{syncId}");Assert.Equal("Succeeded",sync.GetProperty("status").GetString());Assert.Equal(1,sync.GetProperty("processedOrders").GetInt32());Assert.Equal(1,sync.GetProperty("changedOrders").GetInt32());Assert.Equal(1,sync.GetProperty("importedOrders").GetInt32());
        var repeatResponse=await client.PostAsync($"/api/v1/kaspi/connections/{connectionId}/sync",null);Assert.Equal(HttpStatusCode.Accepted,repeatResponse.StatusCode);var repeatId=(await repeatResponse.Content.ReadFromJsonAsync<JsonElement>()).GetProperty("id").GetGuid();await new KaspiSyncWorker(factory.Services.GetRequiredService<IServiceScopeFactory>(),NullLogger<KaspiSyncWorker>.Instance).ProcessOneAsync(CancellationToken.None);var repeat=await client.GetFromJsonAsync<JsonElement>($"/api/v1/kaspi/sync/{repeatId}");Assert.Equal(1,repeat.GetProperty("processedOrders").GetInt32());Assert.Equal(0,repeat.GetProperty("changedOrders").GetInt32());
        ...
        var productDetail=await client.GetFromJsonAsync<JsonElement>($"/api/v1/products/{product.GetProperty("id").GetString()}");var costEntry=Assert.Single(productDetail.GetProperty("costHistory").EnumerateArray());Assert.Equal("CsvImport",costEntry.GetProperty("source").GetString());Assert.Equal("Pilot Owner",costEntry.GetProperty("author").GetString());Assert.Equal(HttpStatusCode.NotFound,(await client.GetAsync("/api/v1/products/not-a-product/costs")).StatusCode);
    ...
    public async Task Owner_Manages_Members_Invitations_And_Role_Boundaries()
    {
        ...
        using var analystClient=factory.CreateClient(new(){AllowAutoRedirect=false,HandleCookies=true});Assert.Equal(HttpStatusCode.OK,(await analystClient.PostAsJsonAsync("/api/v1/auth/login",new{email=memberEmail,password,rememberMe=false})).StatusCode);Assert.Equal(HttpStatusCode.Created,(await analystClient.PostAsJsonAsync("/api/v1/expenses",new{type="Other",amount=100,date="2026-08-12"})).StatusCode);Assert.Equal(HttpStatusCode.Accepted,(await analystClient.PostAsJsonAsync("/api/v1/exports",new{reportType="Products",format="csv"})).StatusCode);Assert.Equal(HttpStatusCode.Forbidden,(await analystClient.GetAsync("/api/v1/kaspi/connections")).StatusCode);Assert.Equal(HttpStatusCode.Forbidden,(await analystClient.GetAsync("/api/v1/telegram")).StatusCode);Assert.Equal(HttpStatusCode.Forbidden,(await analystClient.GetAsync($"/api/v1/organizations/{organizationId}/members")).StatusCode);Assert.Equal(HttpStatusCode.Forbidden,(await analystClient.PostAsJsonAsync("/api/v1/fee-rules",new{scope="Default",valueType="Percentage",value=10,effectiveFrom="2026-08-01"})).StatusCode);
        ...
        using var viewerClient=factory.CreateClient(new(){AllowAutoRedirect=false,HandleCookies=true});Assert.Equal(HttpStatusCode.OK,(await viewerClient.PostAsJsonAsync("/api/v1/auth/login",new{email=memberEmail,password,rememberMe=false})).StatusCode);Assert.Equal(HttpStatusCode.Forbidden,(await viewerClient.DeleteAsync($"/api/v1/organizations/{organizationId}/members/{ownerId}")).StatusCode);Assert.Equal(HttpStatusCode.Forbidden,(await viewerClient.GetAsync($"/api/v1/organizations/{organizationId}/members")).StatusCode);Assert.Equal(HttpStatusCode.Forbidden,(await viewerClient.GetAsync("/api/v1/kaspi/connections")).StatusCode);Assert.Equal(HttpStatusCode.Forbidden,(await viewerClient.GetAsync("/api/v1/telegram")).StatusCode);Assert.Equal(HttpStatusCode.Forbidden,(await viewerClient.GetAsync("/api/v1/fee-rules")).StatusCode);Assert.Equal(HttpStatusCode.Forbidden,(await viewerClient.PutAsJsonAsync($"/api/v1/products/{roleProductId}/status",new{status="Archived"})).StatusCode);Assert.Equal(HttpStatusCode.Forbidden,(await viewerClient.PostAsync("/api/v1/telegram/test",null)).StatusCode);Assert.Equal(HttpStatusCode.Forbidden,(await viewerClient.PostAsync($"/api/v1/kaspi/connections/{Guid.NewGuid()}/sync",null)).StatusCode);
