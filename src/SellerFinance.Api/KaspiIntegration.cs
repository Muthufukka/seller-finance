...
public sealed class KaspiSyncWorker(IServiceScopeFactory scopes,ILogger<KaspiSyncWorker> logger) : BackgroundService
{
    ...
    public async Task ProcessOneAsync(CancellationToken ct)
    {
        ...
        var now=DateTimeOffset.UtcNow;var job=await ClaimAsync(db,now,ct);
        if(job is null){var due=await(from dueConnection in db.MarketplaceConnections.AsNoTracking() join organization in db.Organizations.AsNoTracking() on dueConnection.OrganizationId equals organization.Id join subscription in db.Subscriptions.AsNoTracking() on organization.Id equals subscription.OrganizationId where organization.Status=="Active"&&(subscription.Status==SubscriptionStatus.Active||subscription.Status==SubscriptionStatus.Trialing)&&subscription.PeriodEnd>now&&dueConnection.Status==MarketplaceConnectionStatus.Active&&(!dueConnection.LastSuccessfulSyncAt.HasValue||dueConnection.LastSuccessfulSyncAt<DateTimeOffset.UtcNow.AddMinutes(-15))&&!db.SyncJobs.Any(j=>j.MarketplaceConnectionId==dueConnection.Id&&(j.Status==SyncJobStatus.Queued||j.Status==SyncJobStatus.Running||j.Status==SyncJobStatus.RetryScheduled))&&!db.OrganizationFeatureFlags.Any(f=>f.OrganizationId==dueConnection.OrganizationId&&f.Key=="KaspiSync"&&!f.Enabled) select dueConnection).FirstOrDefaultAsync(ct);if(due is not null){var subscription=await Subscriptions.GetAsync(db,due.OrganizationId,ct);var windowTo=DateTimeOffset.UtcNow;db.SyncJobs.Add(new(){Id=Guid.NewGuid(),OrganizationId=due.OrganizationId,MarketplaceConnectionId=due.Id,WindowFrom=due.LastSuccessfulSyncAt.HasValue?windowTo.AddDays(-14):PlanLimits.InitialHistoryFrom(subscription.Plan,windowTo),WindowTo=windowTo});try{await db.SaveChangesAsync(ct);}catch(DbUpdateException ex)when(DatabaseConflicts.IsActiveSyncJob(ex)){db.ChangeTracker.Clear();}}return;}
        var connection=await db.MarketplaceConnections.SingleOrDefaultAsync(x=>x.Id==job.MarketplaceConnectionId&&x.OrganizationId==job.OrganizationId,ct);if(connection is null){job.Status=SyncJobStatus.RequiresAttention;job.CompletedAt=DateTimeOffset.UtcNow;job.ErrorCode="TENANT_CONNECTION_MISMATCH";await db.SaveChangesAsync(ct);return;}
        KaspiResult result;
    ...
    private static IQueryable<SyncJobEntity> EligibleJobs(SellerFinanceDbContext db,DateTimeOffset now)=>from candidate in db.SyncJobs join organization in db.Organizations on candidate.OrganizationId equals organization.Id join subscription in db.Subscriptions on organization.Id equals subscription.OrganizationId where organization.Status=="Active"&&(subscription.Status==SubscriptionStatus.Active||subscription.Status==SubscriptionStatus.Trialing)&&subscription.PeriodEnd>now&&((candidate.Status==SyncJobStatus.Queued||candidate.Status==SyncJobStatus.RetryScheduled)&&candidate.NextAttemptAt<=now||candidate.Status==SyncJobStatus.Running&&(!candidate.StartedAt.HasValue||candidate.StartedAt<now.AddMinutes(-30)))&&!db.OrganizationFeatureFlags.Any(f=>f.OrganizationId==candidate.OrganizationId&&f.Key=="KaspiSync"&&!f.Enabled) orderby candidate.CreatedAt select candidate;

    private static async Task<SyncJobEntity?> ClaimAsync(SellerFinanceDbContext db,DateTimeOffset now,CancellationToken ct)
    {
        if(db.Database.IsRelational()){var id=await EligibleJobs(db,now).Select(x=>(Guid?)x.Id).FirstOrDefaultAsync(ct);if(!id.HasValue)return null;var staleBefore=now.AddMinutes(-30);var affected=await db.SyncJobs.Where(x=>x.Id==id&&(((x.Status==SyncJobStatus.Queued||x.Status==SyncJobStatus.RetryScheduled)&&x.NextAttemptAt<=now)||x.Status==SyncJobStatus.Running&&(!x.StartedAt.HasValue||x.StartedAt<staleBefore))).ExecuteUpdateAsync(x=>x.SetProperty(y=>y.Status,SyncJobStatus.Running).SetProperty(y=>y.StartedAt,now).SetProperty(y=>y.Attempt,y=>y.Attempt+1),ct);return affected==1?await db.SyncJobs.SingleAsync(x=>x.Id==id,ct):null;}
        await NonRelationalClaimLock.WaitAsync(ct);try{var job=await EligibleJobs(db,now).FirstOrDefaultAsync(ct);if(job is null)return null;job.Status=SyncJobStatus.Running;job.StartedAt=now;job.Attempt++;await db.SaveChangesAsync(ct);return job;}finally{NonRelationalClaimLock.Release();}
    }
