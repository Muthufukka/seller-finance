...
auth.MapPost("/login", async (LoginRequest request, UserManager<AppUser> users, SignInManager<AppUser> signIn, SellerFinanceDbContext db) =>
{
    ...
    var activeOrganization=await(from membership in db.OrganizationUsers.AsNoTracking() join organization in db.Organizations.AsNoTracking() on membership.OrganizationId equals organization.Id where membership.UserId==user.Id&&membership.JoinedAt!=null&&organization.Status=="Active" select organization.Id).FirstOrDefaultAsync();if(activeOrganization is null){await signIn.SignOutAsync();db.AuditLogs.Add(new(){Id=Guid.NewGuid(),UserId=user.Id,Action="auth.login.blocked",EntityType="User",EntityId=user.Id});await db.SaveChangesAsync();return Results.Problem("Доступ к организации приостановлен. Обратитесь в поддержку.",statusCode:403);}
...
api.MapPost("/organizations/{id}/members", async (HttpContext ctx, string id, InviteMemberRequest request, SellerFinanceDbContext db) =>
{
    ...
    var organization=await db.Organizations.AsNoTracking().SingleAsync(x=>x.Id==id);var memberLimit=PlanLimits.MaxMembers(organization.Plan);var members=await db.OrganizationUsers.CountAsync(x=>x.OrganizationId==id&&x.JoinedAt!=null);if(members>=memberLimit)return Results.Problem("Достигнут лимит пользователей тарифа",statusCode:402);
...
api.MapGet("/admin/organizations",async(HttpContext c,SellerFinanceDbContext db,IConfiguration config)=>SaasSecurity.IsAdmin(c.User,config)?Results.Ok(await db.Organizations.AsNoTracking().OrderByDescending(x=>x.CreatedAt).Select(x=>new{x.Id,x.Name,plan=x.Plan.ToString(),x.Status,x.CreatedAt,x.TrialEndsAt,memberCount=db.OrganizationUsers.Count(m=>m.OrganizationId==x.Id&&m.JoinedAt!=null),lastActivity=db.AuditLogs.Where(a=>a.OrganizationId==x.Id).Max(a=>(DateTimeOffset?)a.CreatedAt),lastSync=db.MarketplaceConnections.Where(m=>m.OrganizationId==x.Id).Select(m=>m.LastSuccessfulSyncAt).FirstOrDefault(),lastSyncStatus=db.SyncJobs.Where(j=>j.OrganizationId==x.Id).OrderByDescending(j=>j.CreatedAt).Select(j=>j.Status.ToString()).FirstOrDefault()}).ToArrayAsync()):Results.Forbid());
api.MapPut("/admin/organizations/{id}/plan",async(HttpContext c,string id,AdminPlanRequest request,SellerFinanceDbContext db,IConfiguration config)=>{if(!SaasSecurity.IsAdmin(c.User,config))return Results.Forbid();if(!Enum.TryParse<SubscriptionPlan>(request.Plan,true,out var plan))return Results.BadRequest(new{title="Неизвестный тариф"});var organization=await db.Organizations.SingleOrDefaultAsync(x=>x.Id==id);if(organization is null)return Results.NotFound();organization.Plan=plan;AuditWriter.AddSystem(db,c,id,"saas.plan.changed","Organization",id,$"{{\"plan\":\"{plan}\"}}");await db.SaveChangesAsync();return Results.NoContent();});
...
api.MapPost("/kaspi/sync", async (HttpContext ctx,SellerFinanceDbContext db,CancellationToken ct) =>
{
    ...
    if(!await FeatureFlags.IsEnabledAsync(db,ctx.Tenant(),"KaspiSync",ct))return Results.Problem("Синхронизация отключена администратором SaaS",statusCode:403);
    var connection=await db.MarketplaceConnections.SingleOrDefaultAsync(x=>x.OrganizationId==ctx.Tenant()&&x.Provider=="Kaspi",ct);
    if(connection is null)return Results.NotFound();
...
record AdminPlanRequest(string Plan);
