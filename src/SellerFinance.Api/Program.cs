...
builder.Services.ConfigureApplicationCookie(o =>
{
    ...
    o.Cookie.SecurePolicy = useTestDatabase ? CookieSecurePolicy.SameAsRequest : CookieSecurePolicy.Always;
...
auth.MapPost("/resend-confirmation",async(HttpContext context,ForgotPasswordRequest request,UserManager<AppUser> users,EmailDelivery email,ExternalUrls urls,SellerFinanceDbContext db)=>
{
    var user=await users.FindByEmailAsync(request.Email.Trim());if(user is not null&&!user.EmailConfirmed&&email.IsConfigured){var token=await users.GenerateEmailConfirmationTokenAsync(user);var url=urls.Build($"api/v1/auth/confirm-email?userId={Uri.EscapeDataString(user.Id)}&token={Uri.EscapeDataString(token)}");if(await email.SendAsync(user.Email!,"Подтверждение Seller Finance",EmailDelivery.ConfirmationHtml(url),context.RequestAborted)){var organizationId=await db.OrganizationUsers.Where(x=>x.UserId==user.Id).Select(x=>x.OrganizationId).FirstOrDefaultAsync();db.AuditLogs.Add(new(){Id=Guid.NewGuid(),OrganizationId=organizationId,UserId=user.Id,Action="auth.email.confirmation.resent",EntityType="User",EntityId=user.Id});await db.SaveChangesAsync();}}return Results.Ok(new{message="Если аккаунт ожидает подтверждения, письмо будет отправлено."});
}).RequireRateLimiting("auth");
auth.MapPost("/login", async (LoginRequest request, UserManager<AppUser> users, SignInManager<AppUser> signIn, SellerFinanceDbContext db) =>
{
    ...
    var now=DateTimeOffset.UtcNow;var activeQuery=from membership in db.OrganizationUsers.AsNoTracking() join organization in db.Organizations.AsNoTracking() on membership.OrganizationId equals organization.Id join subscription in db.Subscriptions.AsNoTracking() on organization.Id equals subscription.OrganizationId where membership.UserId==user.Id&&membership.JoinedAt!=null&&organization.Status=="Active"&&(subscription.Status==SubscriptionStatus.Active||subscription.Status==SubscriptionStatus.Trialing)&&subscription.PeriodEnd>now select new{organization.Id,membership.JoinedAt};var preferred=(await users.GetClaimsAsync(user)).FirstOrDefault(x=>x.Type==TenantSecurity.ActiveOrganizationClaim)?.Value;var activeOrganization=!String.IsNullOrWhiteSpace(preferred)?await activeQuery.Where(x=>x.Id==preferred).Select(x=>x.Id).FirstOrDefaultAsync():null;activeOrganization??=await activeQuery.OrderBy(x=>x.JoinedAt).Select(x=>x.Id).FirstOrDefaultAsync();if(activeOrganization is null){await signIn.SignOutAsync();db.AuditLogs.Add(new(){Id=Guid.NewGuid(),UserId=user.Id,Action="auth.login.blocked",EntityType="User",EntityId=user.Id});await db.SaveChangesAsync();return Results.Problem("Доступ к организации или подписке приостановлен. Обратитесь в поддержку.",statusCode:403);}
    await TenantSecurity.SetActiveOrganizationAsync(users,user,activeOrganization);await signIn.SignInAsync(user,request.RememberMe);
    db.AuditLogs.Add(new() { Id=Guid.NewGuid(),OrganizationId=activeOrganization,UserId=user.Id,Action="auth.login",EntityType="User",EntityId=user.Id });await db.SaveChangesAsync();
...
api.MapPost("/organizations/{id}/select",async(HttpContext ctx,string id,SellerFinanceDbContext db,UserManager<AppUser> users,SignInManager<AppUser> signIn)=>
{
    var userId=ctx.User.FindFirstValue(ClaimTypes.NameIdentifier)!;var now=DateTimeOffset.UtcNow;var allowed=await(from membership in db.OrganizationUsers join organization in db.Organizations on membership.OrganizationId equals organization.Id join subscription in db.Subscriptions on organization.Id equals subscription.OrganizationId where membership.UserId==userId&&membership.OrganizationId==id&&membership.JoinedAt!=null&&organization.Status=="Active"&&(subscription.Status==SubscriptionStatus.Active||subscription.Status==SubscriptionStatus.Trialing)&&subscription.PeriodEnd>now select membership).AnyAsync();if(!allowed)return Results.NotFound();var user=await db.Users.SingleAsync(x=>x.Id==userId);await TenantSecurity.SetActiveOrganizationAsync(users,user,id);await signIn.RefreshSignInAsync(user);db.AuditLogs.Add(new(){Id=Guid.NewGuid(),OrganizationId=id,UserId=userId,Action="organization.selected",EntityType="Organization",EntityId=id});await db.SaveChangesAsync();return Results.NoContent();
}).RequireRateLimiting("sensitive");
api.MapPost("/organizations",async(HttpContext ctx,CreateOrganizationRequest request,SellerFinanceDbContext db,UserManager<AppUser> users,SignInManager<AppUser> signIn)=>
{
    var name=request.Name.Trim();if(name.Length is <2 or >120)return Results.BadRequest(new{title="Название должно содержать от 2 до 120 символов"});var userId=ctx.User.FindFirstValue(ClaimTypes.NameIdentifier)!;var organization=new OrganizationEntity{Id=Guid.NewGuid().ToString("N"),Name=name};db.Organizations.Add(organization);db.OrganizationUsers.Add(new(){OrganizationId=organization.Id,UserId=userId,Role=OrganizationRole.Owner,JoinedAt=DateTimeOffset.UtcNow});db.Subscriptions.Add(new(){Id=Guid.NewGuid(),OrganizationId=organization.Id});db.NotificationRules.AddRange(Enum.GetValues<NotificationEventType>().Select(type=>new NotificationRuleEntity{Id=Guid.NewGuid(),OrganizationId=organization.Id,EventType=type,Enabled=true,Threshold=type==NotificationEventType.NegativeMargin?0m:null}));db.AuditLogs.Add(new(){Id=Guid.NewGuid(),OrganizationId=organization.Id,UserId=userId,Action="organization.created",EntityType="Organization",EntityId=organization.Id});await db.SaveChangesAsync();var user=await db.Users.SingleAsync(x=>x.Id==userId);await TenantSecurity.SetActiveOrganizationAsync(users,user,organization.Id);await signIn.RefreshSignInAsync(user);return Results.Created($"/api/v1/organizations/{organization.Id}",new{organization.Id,organization.Name,role="Owner"});
}).RequireRateLimiting("sensitive");
...
api.MapPost("/invitations/accept", async (HttpContext ctx, AcceptInvitationRequest request, SellerFinanceDbContext db,UserManager<AppUser> users,SignInManager<AppUser> signIn) =>
{
    ...
    await db.SaveChangesAsync();
    var appUser=await db.Users.SingleAsync(x=>x.Id==userId);await TenantSecurity.SetActiveOrganizationAsync(users,appUser,invitation.OrganizationId);await signIn.RefreshSignInAsync(appUser);
...
{
    if(!c.Membership().CanWrite())return Results.Forbid();if(!Enum.TryParse<FeeRuleScope>(request.Scope,true,out var scope)||!Enum.TryParse<FeeValueType>(request.ValueType,true,out var type))return Results.BadRequest(new{title="Некорректный тип правила"});if(request.Value<0||type==FeeValueType.Percentage&&request.Value>100)return Results.BadRequest(new{title="Некорректное значение комиссии"});if(request.EffectiveTo.HasValue&&request.EffectiveTo<request.EffectiveFrom)return Results.BadRequest(new{title="EffectiveTo cannot be earlier than EffectiveFrom"});if(scope==FeeRuleScope.Product&&!await db.Products.AnyAsync(x=>x.Id==request.ProductId&&x.OrganizationId==c.Tenant()))return Results.NotFound();var category=request.Category?.Trim();if(scope==FeeRuleScope.Category&&String.IsNullOrWhiteSpace(category))return Results.BadRequest(new{title="Category is required for category rule"});
    if(await FeeRulePeriods.OverlapsAsync(db,c.Tenant(),scope,scope==FeeRuleScope.Product?request.ProductId:null,scope==FeeRuleScope.Category?category:null,request.EffectiveFrom,request.EffectiveTo))return Results.Conflict(new{title="Период пересекается с существующим правилом той же области"});
    var rule=new FeeRuleEntity{Id=Guid.NewGuid(),OrganizationId=c.Tenant(),Scope=scope,ProductId=scope==FeeRuleScope.Product?request.ProductId:null,Category=scope==FeeRuleScope.Category?category:null,ValueType=type,Value=request.Value,EffectiveFrom=request.EffectiveFrom,EffectiveTo=request.EffectiveTo,CreatedByUserId=c.User.FindFirstValue(ClaimTypes.NameIdentifier)!};db.FeeRules.Add(rule);AuditWriter.Add(db,c,"fee.rule.created","FeeRule",rule.Id.ToString(),JsonSerializer.Serialize(new{scope=scope.ToString(),type=type.ToString(),request.Value,request.EffectiveFrom,request.EffectiveTo}));await db.SaveChangesAsync();return Results.Created($"/api/v1/fee-rules/{rule.Id}",new{rule.Id});
});
api.MapPut("/fee-rules/{id:guid}/end",async(HttpContext c,Guid id,FeeRuleEndRequest request,SellerFinanceDbContext db)=>{if(!c.Membership().CanWrite())return Results.Forbid();var rule=await db.FeeRules.SingleOrDefaultAsync(x=>x.Id==id&&x.OrganizationId==c.Tenant());if(rule is null)return Results.NotFound();if(request.EffectiveTo<rule.EffectiveFrom)return Results.BadRequest(new{title="EffectiveTo cannot be earlier than EffectiveFrom"});if(await FeeRulePeriods.OverlapsAsync(db,c.Tenant(),rule.Scope,rule.ProductId,rule.Category,rule.EffectiveFrom,request.EffectiveTo,rule.Id))return Results.Conflict(new{title="Период пересекается с существующим правилом той же области"});rule.EffectiveTo=request.EffectiveTo;AuditWriter.Add(db,c,"fee.rule.ended","FeeRule",id.ToString(),JsonSerializer.Serialize(new{request.EffectiveTo}));await db.SaveChangesAsync();return Results.Ok(new{rule.Id,rule.EffectiveTo});});
...
api.MapGet("/expenses",async(HttpContext c,SellerFinanceDbContext db,DateOnly? dateFrom,DateOnly? dateTo)=>Results.Ok(await db.Expenses.AsNoTracking().Where(x=>x.OrganizationId==c.Tenant()&&(!dateFrom.HasValue||(x.PeriodEnd??x.Date)>=dateFrom)&&(!dateTo.HasValue||x.Date<=dateTo)).OrderByDescending(x=>x.Date).Select(x=>new{x.Id,type=x.Type.ToString(),x.Amount,x.Date,x.PeriodEnd,x.ProductId,x.OrderId,x.Comment,source=x.Source.ToString()}).ToArrayAsync()));
api.MapPost("/expenses",async(HttpContext c,ExpenseRequest request,SellerFinanceDbContext db)=>
{
    if(!c.Membership().CanWrite())return Results.Forbid();if(!Enum.TryParse<ExpenseType>(request.Type,true,out var type)||request.Amount<=0)return Results.BadRequest(new{title="Проверьте тип и сумму расхода"});if(request.PeriodEnd.HasValue&&request.PeriodEnd<request.Date)return Results.BadRequest(new{title="Конец периода не может быть раньше начала"});if(request.PeriodEnd.HasValue&&request.PeriodEnd.Value.DayNumber-request.Date.DayNumber>=366)return Results.BadRequest(new{title="Период расхода не может превышать 366 дней"});if(request.ProductId is not null&&!await db.Products.AnyAsync(x=>x.Id==request.ProductId&&x.OrganizationId==c.Tenant()))return Results.NotFound();if(request.OrderId is not null&&!await db.Orders.AnyAsync(x=>x.Id==request.OrderId&&x.OrganizationId==c.Tenant()))return Results.NotFound();var expense=new ExpenseEntity{Id=Guid.NewGuid(),OrganizationId=c.Tenant(),Type=type,Amount=request.Amount,Date=request.Date,PeriodEnd=request.PeriodEnd,ProductId=request.ProductId,OrderId=request.OrderId,Comment=request.Comment?.Trim(),CreatedByUserId=c.User.FindFirstValue(ClaimTypes.NameIdentifier)!};db.Expenses.Add(expense);AuditWriter.Add(db,c,"expense.created","Expense",expense.Id.ToString());await db.SaveChangesAsync();return Results.Created($"/api/v1/expenses/{expense.Id}",new{expense.Id});
});
...
api.MapPost("/financial-imports/{type}/preview",async(HttpContext c,string type,IFormFile file,FinancialImportService imports,SellerFinanceDbContext db,CancellationToken ct)=>
{
    if(!c.Membership().CanWrite())return Results.Forbid();if(!Enum.TryParse<FinancialImportType>(type,true,out var parsed))return Results.BadRequest(new{title="Тип импорта должен быть Expenses или ActualFees"});
    try{var job=await imports.PreviewAsync(parsed,c.Tenant(),c.User.FindFirstValue(ClaimTypes.NameIdentifier)!,file,ct);var rows=await db.FinancialImportRows.AsNoTracking().Where(x=>x.ImportJobId==job.Id).OrderBy(x=>x.RowNumber).Take(200).ToArrayAsync(ct);AuditWriter.Add(db,c,"financial.import.previewed","FinancialImportJob",job.Id.ToString(),JsonSerializer.Serialize(new{type=parsed.ToString(),job.TotalRows,job.ExpectedChanges}));await db.SaveChangesAsync(ct);return Results.Ok(FinancialImportService.ToPreview(job,rows));}catch(FinancialImportException ex){return Results.BadRequest(new{title=ex.Message});}
}).DisableAntiforgery();
api.MapGet("/financial-imports/{id:guid}",async(HttpContext c,Guid id,SellerFinanceDbContext db)=>{var job=await db.FinancialImportJobs.AsNoTracking().SingleOrDefaultAsync(x=>x.Id==id&&x.OrganizationId==c.Tenant());if(job is null)return Results.NotFound();var rows=await db.FinancialImportRows.AsNoTracking().Where(x=>x.ImportJobId==id).OrderBy(x=>x.RowNumber).Take(200).ToArrayAsync();return Results.Ok(FinancialImportService.ToPreview(job,rows));});
api.MapPost("/financial-imports/{id:guid}/confirm",async(HttpContext c,Guid id,FinancialImportService imports,SellerFinanceDbContext db,CancellationToken ct)=>{if(!c.Membership().CanWrite())return Results.Forbid();try{var applied=await imports.ConfirmAsync(id,c.Tenant(),c.User.FindFirstValue(ClaimTypes.NameIdentifier)!,ct);AuditWriter.Add(db,c,"financial.import.applied","FinancialImportJob",id.ToString(),JsonSerializer.Serialize(new{appliedRows=applied}));await db.SaveChangesAsync(ct);return Results.Ok(new{appliedRows=applied});}catch(KeyNotFoundException){return Results.NotFound();}catch(FinancialImportException ex){return Results.Conflict(new{title=ex.Message});}});
api.MapGet("/financial-imports/{type}/template.xlsx",(string type)=>
{
    if(!Enum.TryParse<FinancialImportType>(type,true,out var parsed))return Results.BadRequest(new{title="Тип импорта должен быть Expenses или ActualFees"});using var book=new ClosedXML.Excel.XLWorkbook();var sheet=book.AddWorksheet(parsed==FinancialImportType.Expenses?"Расходы":"Фактические комиссии");
    var headers=parsed==FinancialImportType.Expenses?new[]{"Type","Amount","Date","PeriodEnd","SKU","OrderExternalId","Comment"}:new[]{"OrderExternalId","LineExternalId","SKU","Amount","ExternalRef"};for(var i=0;i<headers.Length;i++){sheet.Cell(1,i+1).Value=headers[i];sheet.Column(i+1).Width=22;}sheet.Range(1,1,1,headers.Length).Style.Font.Bold=true;
    if(parsed==FinancialImportType.Expenses){sheet.Cell("A2").Value="Advertising";sheet.Cell("B2").Value=15000;sheet.Cell("C2").Value=DateTime.Today;sheet.Cell("D2").Value=DateTime.Today.AddDays(6);sheet.Cell("G2").Value="Недельная кампания";}else{sheet.Cell("A2").Value="KASPI-ORDER-ID";sheet.Cell("B2").Value="KASPI-LINE-ID";sheet.Cell("D2").Value=1250;sheet.Cell("E2").Value="REPORT-2026-08";}
    using var stream=new MemoryStream();book.SaveAs(stream);return Results.File(stream.ToArray(),"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",parsed==FinancialImportType.Expenses?"expense-import-template.xlsx":"actual-fee-import-template.xlsx");
});
api.MapPost("/exports",async(HttpContext c,ExportRequest request,SellerFinanceDbContext db)=>
{
    if(!c.Membership().CanWrite())return Results.Forbid();if(!await FeatureFlags.IsEnabledAsync(db,c.Tenant(),"AdvancedExports"))return Results.Problem("Экспорт отключён администратором SaaS",statusCode:403);if(request.DateFrom.HasValue&&request.DateTo.HasValue&&request.DateFrom>request.DateTo)return Results.BadRequest(new{title="Начало периода не может быть позже конца"});var report=request.ReportType.Trim();if(report.ToLowerInvariant() is not("products" or "orders" or "missingcosts" or "abc")||request.Format.ToLowerInvariant() is not("csv" or "xlsx"))return Results.BadRequest(new{title="Неизвестный отчёт или формат"});var token=TokenTools.CreateToken();var job=new ExportJobEntity{Id=Guid.NewGuid(),OrganizationId=c.Tenant(),CreatedByUserId=c.User.FindFirstValue(ClaimTypes.NameIdentifier)!,ReportType=report,Format=request.Format.ToLowerInvariant(),DateFrom=request.DateFrom,DateTo=request.DateTo,CompleteCostsOnly=request.CompleteCostsOnly,DownloadTokenHash=TokenTools.Hash(token)};db.ExportJobs.Add(job);AuditWriter.Add(db,c,"export.queued","ExportJob",job.Id.ToString(),JsonSerializer.Serialize(new{report,format=job.Format,request.DateFrom,request.DateTo,request.CompleteCostsOnly}));await db.SaveChangesAsync();return Results.Accepted($"/api/v1/exports/{job.Id}",new{job.Id,status=job.Status.ToString(),downloadToken=token,job.ExpiresAt});
}).RequireRateLimiting("sensitive");
...
api.MapPost("/telegram/test",async(HttpContext c,SellerFinanceDbContext db,TelegramClient telegram,CancellationToken ct)=>
{
    if(!c.Membership().CanWrite())return Results.Forbid();var connection=await db.TelegramConnections.AsNoTracking().SingleOrDefaultAsync(x=>x.OrganizationId==c.Tenant()&&x.Status=="Active"&&x.ChatId!=null,ct);if(connection is null)return Results.Conflict(new{title="Telegram не связан"});bool sent;try{sent=await telegram.SendAsync(connection.ChatId!.Value,"Тест Seller Finance: уведомления работают.",ct);}catch(HttpRequestException){sent=false;}catch(OperationCanceledException)when(!ct.IsCancellationRequested){sent=false;}AuditWriter.Add(db,c,sent?"telegram.test.sent":"telegram.test.failed","TelegramConnection",connection.Id.ToString());await db.SaveChangesAsync(ct);return sent?Results.Ok(new{sent=true}):Results.Problem("Telegram API недоступен",statusCode:503);
}).RequireRateLimiting("sensitive");
...
api.MapPut("/admin/organizations/{id}/subscription-status",async(HttpContext c,string id,AdminSubscriptionStatusRequest request,SellerFinanceDbContext db,IConfiguration config,CancellationToken ct)=>
{
    if(!SaasSecurity.IsAdmin(c.User,config))return Results.Forbid();var status=request.Status?.Trim();if(status is not("Active" or "Suspended"))return Results.BadRequest(new{title="Допустимы статусы Active и Suspended"});var suspend=status=="Suspended";if(suspend&&id==c.Tenant())return Results.Conflict(new{title="Нельзя приостановить подписку текущей организации SaaS Admin"});if(suspend&&String.IsNullOrWhiteSpace(request.Reason))return Results.BadRequest(new{title="Укажите безопасную причину приостановки"});var result=await SaasAdminOperations.ChangeSubscriptionStatusAsync(db,id,suspend,ct);if(result.Failure==SubscriptionStatusChangeFailure.NotFound)return Results.NotFound();if(result.Failure==SubscriptionStatusChangeFailure.InvalidTransition)return Results.Conflict(new{title="Переход подписки из текущего статуса недоступен"});if(result.Failure==SubscriptionStatusChangeFailure.PeriodExpired)return Results.Conflict(new{title="Период подписки истёк. Активируйте тариф заново"});var subscription=result.Subscription!;AuditWriter.AddSystem(db,c,id,suspend?"saas.subscription.suspended":"saas.subscription.resumed","Subscription",subscription.Id.ToString(),JsonSerializer.Serialize(new{previousStatus=result.PreviousStatus?.ToString(),status=subscription.Status.ToString(),reason=request.Reason?.Trim()}));await db.SaveChangesAsync(ct);return Results.NoContent();
}).RequireRateLimiting("sensitive");
...
record FeeRuleEndRequest(DateOnly EffectiveTo);
record ActualFeeRequest(decimal Amount,string? Source);
record ExpenseRequest(string Type,decimal Amount,DateOnly Date,DateOnly? PeriodEnd,string? ProductId,string? OrderId,string? Comment);
record ExportRequest(string ReportType,string Format,DateOnly? DateFrom,DateOnly? DateTo,bool CompleteCostsOnly=false);
...
record AdminSubscriptionStatusRequest(string Status,string? Reason);
