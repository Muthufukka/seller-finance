...
api.MapGet("/financial-imports/{type}/template.xlsx",(string type)=>
{
    ...
    var headers=parsed==FinancialImportType.Expenses?new[]{"Type","Amount","Date","SKU","OrderExternalId","Comment"}:new[]{"OrderExternalId","LineExternalId","SKU","Amount","ExternalRef"};for(var i=0;i<headers.Length;i++){sheet.Cell(1,i+1).Value=headers[i];sheet.Column(i+1).Width=22;}sheet.Range(1,1,1,headers.Length).Style.Font.Bold=true;
    if(parsed==FinancialImportType.Expenses){sheet.Cell("A2").Value="Advertising";sheet.Cell("B2").Value=15000;sheet.Cell("C2").Value=DateTime.Today;sheet.Cell("F2").Value="Август";}else{sheet.Cell("A2").Value="KASPI-ORDER-ID";sheet.Cell("B2").Value="KASPI-LINE-ID";sheet.Cell("D2").Value=1250;sheet.Cell("E2").Value="REPORT-2026-08";}
