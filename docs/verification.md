...
Fixture создаёт 10 000 заказов по 10 строк, прогревает согласованный tenant/date snapshot и измеряет повторный расчёт dashboard. Снимок автоматически инвалидируется после `SaveChanges`/`SaveChangesAsync` с бизнес-данными организации; отдельный regression-тест доказывает, что после записи новый заказ попадает в следующий расчёт.
